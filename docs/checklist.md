...
See [17-cyberpunk-fbui.md](17-cyberpunk-fbui.md).
...
| 2026-07-19 | **Stock-Android recipe raid** (`reference/android-capture/`): read working kernel log via `adb logcat -b kernel` (logd CAP_SYSLOG bypasses shell dmesg block). Captured Wi-Fi (docs/21), audio (docs/23: MT6323, only `Speaker_Amp_Switch`+DAPM, `hw:0,5`/`hw:0,1`), camera (docs/22: **SP2509** 2MP i2c 0x7a). Board=`hcn/mid721l_96e_mipi`, nvram@0x400000 (Wi-Fi MAC redacted). chipId=0 is a RED HERRING (prints on working Android). |
...
| 2026-07-19 | **★ WI-FI WORKS + SSH — cable retired (docs/24 devlog).** SDIO wall was OUR bug: MT8127 consys is on-die, uses **BTIF** not SDIO — STP mode `0x23` not `0x24`. Kernel asks userspace for the patch (`srh_patch` on /dev/stpwmt); `wmtctl2.c` answers (SET_PATCH_NUM/INFO + "ok") → `patch dwn ok` → `wlan probe ok` → **wlan0**. Static musl **wpa_supplicant 2.11 (nl80211+libnl-tiny**; wext associates but never completes handshake) → joined the lab Wi-Fi, **192.168.4.32**. Static **dropbear 2022.83**: `ssh root@192.168.4.32` passwordless, scp works. `tools/net-push.py` = transfer over wifi (serial ACM wedges under load). NEONWIRE NETWORK panel now a full touch Wi-Fi manager (scan list + on-screen keyboard). |
| 2026-07-19 | **Clean git repo + sync scaffolding.** `git init` → **github.com/nukleas/neonwire-os** (125 source files, 3.9M; binaries/kernels/blobs gitignored). `tools/publish.sh` (build→GitHub release) + `experiments/net/neon-sync.sh` (device wgets artifacts, self-applies) for hands-off updates. Gotchas found: 2× neui = framebuffer flashing (one instance only); loadavg ~8 is D-state kernel threads, not CPU; telnetd/dropbear need devpts mounted or they serve once. |
